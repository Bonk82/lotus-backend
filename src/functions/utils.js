...
export const generarCRUDPostgres = function (datos, respuesta) {
  console.log('generarCRUDPostgres',datos.query);
  
  const { opcion,base,esquema,tabla } = datos.query;
  let q =``;
  if(opcion=='POSTGRES') q = `SELECT column_name campo, data_type tipo FROM information_schema.columns
  WHERE table_schema = '${esquema}' AND table_name = '${tabla}' order by ordinal_position;`;
  console.log(q);

  const cnx = (base == 'seguimiento') ? connseguimiento : conn;

  cnx.obtener(function (er, cn) {
      cn.query(q, async function (error, resultado) {
          cn.release();
          if (error) respuesta.send([{ message: "Error: " + error }]);
          if (!error){
              const data = await armarCRUD(resultado.rows,esquema,tabla,cnx)
              console.log('ya casi',data.message);
              respuesta.send(data);
          }
    });
  });
...
const armarCRUD =(campos,esquema,tabla,cnx)=>{
  return new Promise((resolve, reject) => {
      ...
      declare
          ...
          BEGIN
              if (operacion='I') then
                  ...
                  values (#VALORES_INSERT# now()::timestamp, _usuario_registro,null,null,1) RETURNING ${campos[0].campo} INTO id;  
              ...
              if (operacion='D')then
                  update ${esquema}.${tabla} set ${campos[campos.length-1].campo} = 0,fecha_modifica=now()::timestamp,usuario_modifica=_usuario_registro
                  where ${campos[0].campo}  = _${campos[0].campo};
      ...
      let campos_insert = campos.slice(1).map(campos =>campos.campo).join(', ');

      let valores_insert = ''
      campos_in.forEach(e=>{
          valores_insert += `${['character varying','text'].includes(e.tipo) ? 'UPPER(':''} _${e.campo} ${['character varying','text'].includes(e.tipo) ? ')':''}, `
      })
      ...
      cnx.obtener(function (er, cn) {
          cn.query(plantilla, async function (error, resultado) {
              cn.release();
              if (error){
              ...
              if (!error){
                  ...
                  armado = {template:plantilla,message:'Funcion CRUD creada Satisfactoriamente'}
                  resolve(armado)
              } 
        });
      });
